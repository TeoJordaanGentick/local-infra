...
> *one* app at a time. teo-infra breaks that: it owns the shared services once,
...
split** here: teo-infra *is* the provider, so from its own standpoint every
...
teo-infra, like `gentick-infra`, is a pure provider with one compose file.)
...
```bash
cd /c/dev/teo-infra           # (WSL: /mnt/c/dev/teo-infra — docker lives in WSL)

./svc-build-env.sh            # 1. build .env from .env.example + the pragma creds
./boot.sh                     # 2. render the broker password file, then start
```

`boot.sh` does two steps in order: `gen-nanomq-pwd.sh` (renders
`nanomq/nanomq_pwd.conf` from `.env` — NanoMQ has no env templating) then
`svc-start.sh`. To do it by hand:
...
`wsl -e bash -lc 'cd /mnt/c/dev/teo-infra && ./boot.sh'`.
...
nginx routes by `server_name`. **`localhost` works with no hosts-file edit** and
is the talosot default; the `dev.<app>.local` names are optional (see below).
...
| `http://dev.talosot.local/` | same as above | `talosot-api:8080` | same |
| `http://dev.rms.local/` | rms React SPA (`nginx/html/rms`) | `rms-api:8080` | — |
...
### Optional: `dev.<app>.local` hostnames

`localhost` always serves talosot. To reach a *second* app by name (and to have
per-app hostnames), add one line to your hosts file — this needs **admin** and
is **per-developer**, so it is not in the repo:

- Windows: `C:\Windows\System32\drivers\etc\hosts`
- WSL/Linux: `/etc/hosts`

```
127.0.0.1  dev.talosot.local dev.rms.local
```

Without it, `http://localhost/` still serves talosot fully — the hostnames are a
convenience, never a requirement. To make a *different* app the `localhost`
default, move `default_server` to its `listen` line in `nginx/nginx.conf`.

---

## Switching an app from its standalone stack to teo-infra
...
**After (alongside teo-infra):**

1. Bring up the shared services **first**:
   ```bash
   cd /c/dev/teo-infra && ./boot.sh
   ```
2. Put the talosot web bundle where teo-infra's nginx serves it — build it into
   `teo-infra/nginx/html/talosot/` (and the emulator into
   `nginx/html/talosot-emulator/`), or copy an existing build in. (The agollum
   Flutter build script is the house tool for this — it already targets a
   shared nginx html dir for `gentick-infra`; point it at teo-infra's.)
...
`SHARED_NETWORK=backend-net`. teo-infra registers exactly those names. That is
...
> teo-infra is up.** That starts a *second* `postgres`/`mqtt` on the same
...
The `dev.rms.local` server block in `nginx/nginx.conf` is the template. To wire a
new app `foo`:

1. **nginx** — copy the rms `server { … }` block, then change:
   - `server_name dev.foo.local;`
...
2. **hosts** — add `dev.foo.local` to the hosts line (optional, admin).
3. **bundle** — create `nginx/html/foo/` and put the app's web build there.
4. **credentials** — if `foo` needs a service teo-infra doesn't provide yet, add
...
- **Host level.** Everything persistent is under `${HOST_DATA_ROOT}` (default
  ...
  from gone. Because it is the same root the app stacks use, teo-infra serves the
...
- **Nothing secret is committed.** `.env` and `nanomq/nanomq_pwd.conf` are
  gitignored; only `.env.example` and `nanomq_pwd.conf.example` are tracked.
...
if it is missing, and everyone else looks it up. Bring **teo-infra up first** on
...
backend-net`, start teo-infra first.
...
| `gen-nanomq-pwd.sh` | Render `nanomq/nanomq_pwd.conf` from `.env` (teo-infra-specific) |
| `boot.sh` | Convenience: `gen-nanomq-pwd.sh` then `svc-start.sh` |

**Scope note.** teo-infra has only `compose.yml`, so use the default
...
teo-infra/
  ...
  boot.sh                     gen pwd file, then svc-start
  ...
  nanomq/
    nanomq.conf  nanomq_acl.conf         broker config + Sparkplug ACL
